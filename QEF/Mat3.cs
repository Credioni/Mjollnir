...
namespace QEFSolverMDC {
	public class Mat3
	{
		...
		public float Fnorm()
		{
			return (float)Mathf.Sqrt((m00 * m00) + (m01 * m01) + (m02 * m02)
		...
		public float Off()
		{
			return (float)Mathf.Sqrt((m01 * m01) + (m02 * m02) + (m10 * m10)
		...
		public static void CalcSymmetricGivensCoefficients(float a_pp, float a_pq, float a_qq, out float c, out float s)
		{
			...
			float stt = (float)Mathf.Sqrt(1.0f + tau * tau);
			float tan = 1.0f / ((tau >= 0) ? (tau + stt) : (tau - stt));
			c = 1.0f / (float)Mathf.Sqrt(1.0f + tan * tan);
			s = tan * c;
		}
	}
}
