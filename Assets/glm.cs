...
public static class glm
{
    ...
    public static float FractalNoise(int octaves, float frequency, float lacunarity, float persistence, Vector2 position)
    {
        ...
        {
            noise += Code.Noise.Noise.Perlin(p.x, p.y) * amplitude;
            p *= lacunarity;
    ...
    public static float Density_Func(Vector3 worldPosition)
    {
        ...
        /*
        int test = 0;
        Vector3 offset = new Vector3(10000* test,10000 * test,10000 * test);
        float noiseX = Mathf.Abs((float)(worldPosition.x + offset.x) / 10);
        float noiseY = Mathf.Abs((float)(worldPosition.y + offset.y) / 10);
        float noiseZ = Mathf.Abs((float)(worldPosition.z + offset.z) / 10);
        float noise = SimplexNoise.Noise.Generate(noiseX,noiseY,noiseZ);
        if(noise == 0){
            count++;
        }
        if(count == 1000){
